...
import {
  ...
  Disabled,
  ReadOnly,
} from "@/stories/date-time-input.stories";
...
describe("<DateInput />", () => {
  ...
  it("should accept a date time string as value", async () => {
    ...
    const input = await screen.getByLabelText("Published at");
    await expect.element(input).toHaveValue("2021-09-11T08:51");
  });
  ...
  it("should accept a date object as value", async () => {
    const screen = render(
      <Basic
        simpleFormProps={{
          defaultValues: { publishedAt: new Date("2021-09-11T06:51:17.772Z") },
        }}
      />,
    );
    const input = await screen.getByLabelText("Published at");
  ...
  it("should display validation error", async () => {
    const screen = render(
      <Basic
        ...
        dateTimeInputProps={{ validate: required() }}
      />,
    );
    const input = await screen.getByLabelText("Published at *");
    await input.click();
  ...
  describe("error message", () => {
    it("should not be displayed if field is pristine", async () => {
      const screen = render(
        <Basic dateTimeInputProps={{ validate: required() }} />,
      );
