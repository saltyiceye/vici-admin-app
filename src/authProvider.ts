...
export const USER_STORAGE_KEY = 'vicisms-xyz-mgniSession';
export const apiUri = 'http://localhost:8080/v1';

// const accessControlStrategies = {
//     admin: ({ resource, action }: { resource: string; action: string }) => {
//         return true;
//     },
//     tech: ({ resource, action }: { resource: string; action: string }) => {
//         return resource !== 'users';
//     },
//     customer: ({ resource, action }: { resource: string; action: string }) => {
//         return resource === 'sdr' && action === 'list';
//     },
// }
...
export const authProvider: AuthProvider = {
    login: async (params) => {
        ...
        const auth = await fetchUtils.fetchJson(`${apiUri}/login`, options);
        const { json, status }  = auth;
        
        if (status == 200) {
            localStorage.setItem(USER_STORAGE_KEY, json.token);
    ...
    checkAuth: () =>
        localStorage.getItem(USER_STORAGE_KEY) ? Promise.resolve() : Promise.reject(),
    ...
    getIdentity: async () => {
        ...
        // const auth = await fetchUtils.fetchJson(`${apiUri}/getInfo`, options);
        // const { json }  = auth;
        const token = localStorage.getItem(USER_STORAGE_KEY) ? localStorage.getItem(USER_STORAGE_KEY) : "";
        const user = jwtDecode<CustomJwtPayload>(token ?? '');
